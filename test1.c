...
bool collision_pave(conteneur block_x, conteneur block_y, int tete_x, int tete_y);
...
int main()
{
	...
	{	
		...
		for(int j=0; i<TAILLE_PAVE;j++)
		{
			for(int h=0; h<TAILLE_PAVE;h++)
			{
				printf("%d    ",pos_des_paves_x[i][j][h]);
			}
			printf("\n");
		}
		printf("pavé %d\n\n",i);
...
void toutes_coord_tab (conteneur tab_coords_blocs)
{
    int lesPavesX[NB_PAVES] = { 3, 74, 3, 74, 38, 38};
	int lesPavesY[NB_PAVES] = { 3, 3, 34, 34, 21, 15};
    coord_blocs bloc;
    for(int i=0; i<NB_PAVES; i++)
    {
        tab_coords_blocs[i]=coord_paves(bloc,lesPavesX[i],lesPavesY[i]);
    }
}

void coord_paves (coord_blocs tab_bloc, int coord_dep_x, int coord_dep_y)
{
